...
export function TimePickerField({
  ...
  return (
    <View>
      ...
      {presets.length > 0 && (
        <View className="flex-row flex-wrap" style={{ gap: theme.spacing.sm, marginTop: theme.spacing.sm }}>
          {presets.map((preset) => {
            const selected = preset.hour === hour && preset.minute === minute;
            return (
              <Pressable
                key={`${preset.hour}:${preset.minute}:${preset.label}`}
              ...
              >
                <Text style={[theme.typography.label, { color: selected ? theme.colors.brand : theme.colors.textSecondary }]}>
                  {preset.label}
                </Text>
              </Pressable>
            );
