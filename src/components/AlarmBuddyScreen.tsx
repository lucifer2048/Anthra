    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.canvas }} edges={["top", "bottom"]}>
      ...
      >
        <ScreenHeader
          eyebrow="ORGANIZE"
          title="Alarms"
          subtitle="Motion-verified wakeups · India Standard Time"
          onBack={onBack}
      ...
      >
        <Card variant="brand" padding="large">
          <View className="flex-row items-start" style={{ gap: spacing.md }}>
            <View
              ...
              style={{ width: 52, height: 52, borderRadius: radii.lg, backgroundColor: colors.surface }}
            >
              <AlarmClock accessible={false} color={colors.brand} size={28} />
            </View>
            <View className="min-w-0 flex-1">
              <Text style={[typography.titleMedium, { color: colors.textPrimary }]}>Push-ups dismiss the alarm</Text>
              <Text style={[typography.label, { color: colors.brand, marginTop: spacing.xs }]}>
                {enabledCount} {enabledCount === 1 ? "alarm" : "alarms"} active
              </Text>
            </View>
          </View>
          <Text style={[typography.body, { color: colors.textSecondary, marginTop: spacing.lg }]}>
            Anthra verifies your side-view movement on this device while the alarm rings. Camera frames are never saved or uploaded.
          </Text>
        </Card>
        ...
        <Card style={{ marginTop: spacing.lg }}>
          <Pressable
            ...
            accessibilityLabel={setupGuideExpanded ? "Hide alarm setup guide" : "Show alarm setup guide"}
            onPress={() => setSetupGuideExpanded((expanded) => !expanded)}
            className="flex-row items-center"
            style={{ minHeight: layout.compactTouchTarget }}
          >
            <View className="min-w-0 flex-1">
              <Text style={[typography.label, { color: colors.brand }]}>BEFORE YOU RELY ON AN ALARM</Text>
              <Text style={[typography.body, { color: colors.textSecondary, marginTop: spacing.xs }]}>
                {setupGuideExpanded ? "Hide camera placement guidance" : "Review camera placement and test tracking"}
              </Text>
            </View>
            {setupGuideExpanded
              ? <ChevronUp accessible={false} color={colors.brand} size={22} />
              : <ChevronDown accessible={false} color={colors.brand} size={22} />}
          </Pressable>
          {setupGuideExpanded && (
            <View style={{ gap: spacing.sm, marginTop: spacing.md }}>
              <Text style={[typography.body, { color: colors.textSecondary }]}>1. Turn the phone landscape and prop it securely beside you, with its long edge parallel to the floor.</Text>
              <Text style={[typography.body, { color: colors.textSecondary }]}>2. Start around 1–1.5 metres from your side and 30–60 cm above the floor, then adjust until head through knees fit.</Text>
              <Text style={[typography.body, { color: colors.textSecondary }]}>3. Keep your head, visible arm, hips, and camera-side knee inside the frame. Ankles and feet are optional.</Text>
              <Text style={[typography.body, { color: colors.textSecondary }]}>4. Face either left or right, plant hands and feet, and hold a straight-arm plank until the tracker says Ready.</Text>
              <Text style={[typography.caption, { color: colors.textTertiary }]}>Compact mode checks elbow depth, chest travel, body alignment, and the planted hand. Visible feet enable extra leg and foot checks.</Text>
            </View>
          )}
          <Button
        ...
        {permissionStatus && setupNeedsAttention && (
          <Card variant="subtle" style={{ marginTop: spacing.lg }}>
            <StatusBanner
        ...
        {notice && (
          <StatusBanner
            ...
            message={notice}
            variant={noticeNeedsAttention ? "warning" : "success"}
            style={{ marginTop: spacing.lg }}
          />
        )}
            ...
            <Card padding="none" style={{ overflow: "hidden" }}>
              {history.slice(0, 8).map((entry, index) => (
                ...
                >
                  ...
                  <Text style={[typography.label, { color: entry.status === "completed" ? colors.success : colors.warning, textAlign: "right" }]}>
        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"}>
          <View style={{ flex: 1, justifyContent: "flex-end", backgroundColor: colors.scrim }}>
            ...
            >
              <View className="flex-row items-start" style={{ gap: spacing.md }}>
                <View className="min-w-0 flex-1">
                  <Text accessibilityRole="header" style={[typography.titleLarge, { color: colors.textPrimary }]}>{form.id ? "Edit alarm" : "New alarm"}</Text>
                  <Text style={[typography.body, { color: colors.textSecondary, marginTop: spacing.xs }]}>All alarm times use India Standard Time.</Text>
                </View>
                <Button label="Close" variant="ghost" size="small" disabled={saving} onPress={() => setEditorOpen(false)} />
              </View>
              ...
              >
                {editorError && (
                  <StatusBanner title="Check the alarm" message={editorError} variant="danger" style={{ marginBottom: spacing.lg }} />
                )}
                ...
                <View style={{ marginTop: spacing.lg }}>
                  <TimePickerField
                    label="Alarm time · India Standard Time"
                    hour={form.hour}
                    minute={form.minute}
                ...
                <TextField
                  ...
                  helperText="Choose a target from 1 to 100."
                ...
                >
                  ...
                  <View className="min-w-0 flex-1">
                    ...
                    <Text style={[typography.caption, { color: colors.textSecondary, marginTop: spacing.xs }]}>{soundPicking ? "Opening device sounds…" : "Choose an installed alarm sound"}</Text>
                  </View>
                </Pressable>

                <Button
                  label={saving ? "Saving…" : form.enabled ? "Save and schedule" : "Save changes · keep off"}
                  loading={saving}
                  fullWidth
                  onPress={() => handleSave().catch(() => undefined)}
                  style={{ marginBottom: spacing.md, marginTop: spacing["2xl"] }}
                />
              </KeyboardAwareScrollView>
            </View>
          </View>
